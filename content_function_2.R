...
var_table <- load_table_by_variable_monitor(138)
var_table$daten <- rename(var_table$daten, "Jahr" = Zeit, "Wert" = wert, "Messeinheit" = einheit)
  ...
  fluidPage(
    column(
      ...
      selectInput(
        ...
        choices = c("---", setdiff(unique(colnames(var_table$daten)), c("id", "variable", "Jahr", "Wert", "Messeinheit")))
      ),
      conditionalPanel(
    ...
    column(
      width = 9,
      conditionalPanel(
        condition = "input.darstellungsweise1_in == 'Karte'",
        numericInput(
          inputId = "karte1_jahr",
          label = "Jahr:",
...
server <- function(input, output, session) {
  ...
  filtered_data_gliederung_kategorie_karte <- reactive({
    ...
    if (input$darstellungsweise1_in == "Karte") {
      return(filtered_data_gliederung_kategorie() %>% filter(.[["Jahr"]] == as.numeric(input$karte1_jahr)))
    } else {
      return(NULL)
    }
  })
  ...
  output$output_list <- renderUI({
    ...
    } else if (input$darstellungsweise1_in == "Karte" & nrow(plot_data() > 0)) {
