...
# unfortunately, box currently does not allow to give a handle to both module name and function name
tryCatch({
  box::use(svMagpie[db_connect = connect_magpie])
  Sys.setenv("magpie_user" = "magpie_read")
  Sys.setenv("magpie_db" = "produktiv")
  use_sv_magpie <- TRUE
},
error = function(cond) {
  box::use(. / utils[db_connect = raise_invalid_connection_error])
  use_sv_magpie <- FALSE
})
...
get_db_data_unique <- function(data_long) {
  ...
  if (print_messages)
    message(paste0("Found ", nrow(long_unique), " unique entries to search in."))
  return(long_unique)
}
